...
void loop()
{
  ...
  if (event_dist) {
    ...
    // bang bang control
    // 저는 서보 암과 경첩이 안쪽으로 향하도록 연결했습니다. 그래서 원하는 결과를 얻으려면 아래 식을 반대로 수정해야 했습니다.
    if (dist_ema > _DIST_TARGET) {
      duty_target = _DUTY_NEU + _BANGBANG_RANGE; // -를 +로 변경
...
float volt_to_distance(int a_value)
{
  ...
  return 791 + -3.53 * a_value + 5.61E-03 * a_value * a_value + -3.16E-06 * a_value * a_value * a_value ;
}
